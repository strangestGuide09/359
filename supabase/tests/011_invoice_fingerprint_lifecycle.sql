...
create temporary table fingerprint_test_ids(
...
reset role;
insert into fingerprint_test_ids(name,id)
select 'reliable_purchase',purchase_id
from public.invoice_imports
where household_id='d2000000-0000-0000-0000-000000000001'
  and exact_pdf_hash=repeat('a',64);
set local role authenticated;
set local request.jwt.claim.sub='d1000000-0000-0000-0000-000000000001';
...
select is((select duplicate_status from public.find_invoice_duplicate('d2000000-0000-0000-0000-000000000001',repeat('a',64),repeat('b',64))),'linked_archived_not_authorized','ordinary archive remains protected from non-payer restoration');
select ok((select duplicate_status from public.find_invoice_duplicate('d2000000-0000-0000-0000-000000000001',repeat('a',64),repeat('b',64))) <> 'none','ordinary archive never appears permanently deleted');
reset role;
set local role authenticated;
set local request.jwt.claim.sub='d1000000-0000-0000-0000-000000000001';
select lives_ok($$select public.purge_purchase_receipt((select id from fingerprint_test_ids where name='reliable_purchase'))$$,'owner permanently purges linked receipt');
...
select is((select count(*)::integer from public.purchase_items where purchase_id=(select id from fingerprint_test_ids where name='reliable_purchase')),0,'permanent purge leaves no reviewed item rows');
select is((select count(*)::integer from public.ledger_activity where subject_id=(select id from fingerprint_test_ids where name='reliable_purchase')),0,'permanent purge leaves no receipt-specific activity payload');
...
select is((select duplicate_status from public.find_invoice_duplicate('d2000000-0000-0000-0000-000000000001',repeat('4',64),repeat('3',64))),'none','permitted exact legacy cleanup removes the original hash-pair match');
reset role;
set local role authenticated;
set local request.jwt.claim.sub='d1000000-0000-0000-0000-000000000002';
select throws_ok($$
  select public.release_orphaned_invoice_fingerprints('d2000000-0000-0000-0000-000000000001',repeat('7',64),repeat('8',64))
$$,'P0001','Only the household owner can release an orphaned fingerprint','partner cannot erase an orphaned exact reservation');
reset role;
set local role authenticated;
set local request.jwt.claim.sub='d1000000-0000-0000-0000-000000000001';
