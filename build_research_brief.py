...
add_body(doc,'Magic-link authentication is unsuitable as the only web path when a person may read email in one application but want the session in another browser. The shared web client therefore now supports email one-time-code verification: each person requests a code, enters it in the chosen browser, and receives a session there. The Supabase Magic Link email template must use {{ .Token }} (not {{ .ConfirmationURL }}) for Supabase to send a code. This improves cross-device usability without widening database access or collecting additional ledger data.', 'Cross-browser sign-in update — 13 July 2026, 14:50 IST: ')
