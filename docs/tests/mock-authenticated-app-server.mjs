...
globalThis.__GROCERY_LEDGER_TEST_CLIENT__ = (() => {
  ...
  const purchases = [
    { id:"july", household_id:"home", label:"Instamart", amount:882.99, paid_by:"ritesh", purchased_on:"2026-07-03", created_at:"2026-08-13T10:00:00Z", archived_at:scenario==="archived"?"2026-08-20T10:00:00Z":null, is_personal:false, purchase_items:["Til Black","Gooseberry","Fresh Turmeric","Avocado","Eggs","Lemon","Toothpaste","Bread","Papad","Boondi","Chamomile Tea"].map((name,index)=>({id:"j"+(index+1),name,quantity:1,line_total:[89,30,14,69,280,14,112,49,70,75,69][index],shared_line_total:[89,30,14,69,280,14,112,49,70,75,69][index],item_kind:"product",include_in_total:true,is_personal:false,is_tracked_for_restock:true,display_order:index})).concat([{id:"j12",name:"Handling fee",quantity:1,line_total:11.99,shared_line_total:11.99,item_kind:"fee",include_in_total:true,is_personal:false,is_tracked_for_restock:false,display_order:11}]) },
  ...
  if(scenario==="empty") purchases.splice(0);
  ...
  const query = table => {
    ...
    const q={ select(){return q}, eq(field,value){filters.push([field,value]);return q}, is(field,value){if(field==="archived_at"&&value===null) archived="active";return q}, not(field,op,value){if(field==="archived_at") archived="archived";return q}, maybeSingle(){single=true;return q}, then(resolve){let data=[...(rows[table]||[])].filter(row=>filters.every(([field,value])=>row[field]===value)); if(archived==="active")data=data.filter(row=>row.archived_at==null); if(archived==="archived")data=data.filter(row=>row.archived_at!=null); return Promise.resolve({data:single?(data[0]||null):data,error:scenario==="error"&&table==="purchases"?{message:"Temporary mocked ledger failure"}:null}).then(resolve)} };
    return q;
  };
