...
function panelSignedOut() {
  ...
  setPanel(`<div><p>SHARED ACCESS</p><h2>Sign in to your shared ledger</h2><small>We send a one-time sign-in link. No app password, receipt, payment detail, or address is stored.</small></div><form id="login-form"><label>Email<input id="login-email" type="email" required autocomplete="email"></label><button>Send sign-in link</button></form>`);
  $("login-form").onsubmit = async event => { event.preventDefault(); const email = $("login-email").value.trim(); const { error } = await supabase.auth.signInWithOtp({ email, options: { emailRedirectTo: location.href } }); note(error ? error.message : "Check your email for the sign-in link."); };
}
