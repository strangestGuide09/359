...
heading(doc,'14 July 2026, 16:15 IST — Dedicated sign-in gate and persistent-session verification')
body(doc,'What changed: simplified the unsigned experience to a compact Grocery Ledger account gate. It contains only sign-in/sign-up by email and the privacy boundary; no ledger, household, invitation, expense, or settlement controls render behind it. Once authenticated, a person goes to the household picker or their selected ledger. Invitations remain a small action inside the selected household settings, not an onboarding window.')
body(doc,'Session behaviour: Supabase session persistence and token refresh are now explicitly configured with browser local storage. A successful sign-in survives reloads, tab switches, and normal browser reopening; a person should need to sign in again only after choosing Sign out, clearing the site data, or an expired/revoked authentication session. This is intentionally more convenient than ending the session merely because a tab closes.')
body(doc,'Verification: JavaScript syntax validation, four static flow/privacy/lifecycle tests, and a local browser visual check passed. The verified signed-out preview contains only the centred account gate and privacy footer. The public GitHub Pages site will show this correction after the commit is pushed and Pages finishes deploying.')

heading(doc,'14 July 2026, 16:35 IST — Project-document responsibility corrected')
body(doc,'Decision: GroceryLedger Change Log is the sole dated implementation record. It carries code, migration, test, release, defect, and verification details. Grocery Slip & Shared Household Tracker is a stable product/design reference instead: research, requirements, flows, privacy policy, ERD/DRD, role model, lifecycle rules, and critical product decisions.')
body(doc,'Action: rebuilt the Tracker to remove the running implementation timeline and preserve the durable product/design material. Verification: both documents were regenerated and rendered for visual review.')
doc.save(OUT)
print(OUT)
